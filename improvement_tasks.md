...
### [x] T1. Feedback sanitizer misses JFROG / Atlassian / Bearer tokens
...
### [x] T2. Token value shell-evaluated when preflight sources `.env.*` (local RCE)
...
### [x] T3. `git clone` on attacker-controlled repo URL, no transport allowlist (local RCE)
...
### [x] T4. JS helpers interpolate pkg/version into `execSync` (shell injection)
...
### [x] T5. No tests for newest safety-critical code
...
### [x] T6. References use obsolete flat script paths in runnable blocks
...
### [x] T7. `recommended_strategy` schema divergence (JS returns null / empty)
...
### [x] T8. `detect_env_go.sh` can emit invalid JSON for vendor count
...
### [x] T9. provenance validator can false-block legitimate 0-breaking-change upgrades
...
### [x] T10. Jira REST scripts: no host allowlist (SSRF + token to arbitrary host)
...
### [x] T11. CI doesn't run mypy; pyproject URLs are placeholders
...
### [x] T12. Installers use bare `pip install` (PEP 668 failure)
...
### [x] T16. Dead link in report template
...
### [x] T17. Truncated sentence in SKILL.md Phase 2.1
