...
- **Phase 2** decides upgrade strategy from the dep tree: `direct_bump` / `lock_only` / `bump_parent` / `pin_add` / `pin_update` / `pin_source` (`pin_source` is Python/Go only). Each strategy also carries a `mechanism` string (e.g. `npm-overrides`, `uv-override`, `go-replace`) for the language-specific tool. Go adds MVS-aware variants (`bump_indirect`, `major_version_rewrite`). The strategy chosen here drives the Phase 5 commands.
