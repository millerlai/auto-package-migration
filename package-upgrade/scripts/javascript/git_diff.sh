...
# SECURITY: repo URLs come from package-registry metadata (attacker-controllable).
# git's ext:: transport executes arbitrary commands and file:// reads local
# paths, so refuse anything that isn't https.
case "$REPO_URL" in
    https://*) ;;
    *)
