...
description: >
  升級 Python / JavaScript / TypeScript 套件或修復 CVE 漏洞的完整工作流。
  ...
  JavaScript/TypeScript: MVP 支援 npm + TypeScript .d.ts API surface diff
  (yarn / pnpm / bun 後續 stage)。
...
先看專案根目錄有哪些訊號檔，**Python 與 JavaScript 同時存在時優先 JS**
(因為 Python 全棧專案常常不會有 `package.json`，但前端專案可能順手放個 `setup.py`)：

| 訊號 | 語言 |
|------|------|
| `package.json` | JavaScript / TypeScript |
| `pyproject.toml`、`requirements.txt`、`setup.py`、`Pipfile`、`uv.lock`、`poetry.lock` (且無 `package.json`) | Python |
| 兩者皆有 | 詢問使用者要升的是哪一邊的套件 |

`language ∈ {python, javascript}` 要在 session 中保留，後續所有 phase 都會用到。
...
### Step 0.3: Pre-flight checks（JS path 必跑）

**JS path 在 Phase 1 之前必須跑 pre-flight**，把所有可能 block 的環境問題一次列出。
...
- `workspace` → memory: 「此 repo 是 workspace monorepo，升級需考慮哪個 workspace 是目標」
...
### Step 3.0: (JS only) .d.ts API Surface Diff

僅當 `language == "javascript"` 時執行 (Python path 跳過此步)：
...
詳細策略與信心分數規則見 `references/js_ast_strategy.md`。
...
若 `strategy == "none"` 表示新舊版都沒有 .d.ts 也讀不出 JS exports — 不要硬要報告
「沒有 breaking change」，改告知使用者並依靠 Git Diff + Changelog 雙軌做判斷。
...
兩個 script 的輸出 schema 對齊，包含每個來源檔案中:
- import / require 該 package 的位置與形式
- 使用的 symbol (含完整 chain, 如 `axios.default.get`)
- 行號 + 周圍 ±5 行的程式碼上下文
...
### Step 5.4: (JS only) Post-edit lockfile 離線驗證

只要 JS path 走過「手動編輯 lockfile」這條 fallback 路徑（preflight 缺 auth token、
...
純本地操作，不需要 token / network。若 checksum 抄錯會在這步擋下，不會等到 CI 才炸
（IMPROVEMENTS #8）。驗證失敗 → **不要 commit**，回 Phase 5.3 重做。
