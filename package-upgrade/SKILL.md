...
```bash
bash scripts/detect_env_js.sh <project_path>
```
...
```bash
bash scripts/detect_env_go.sh <project_path>
```
...
接著一律讀 `references/go_workflow.md` 為主文件，並依場景補讀：
...
```bash
bash scripts/preflight_go.sh <project_path>
```
...
**若只抽到 `BDSA-...`**（內部編號，外部不可查）→ 走 `references/bdsa_mapping.md` 的
...
```bash
bash scripts/govulncheck_go.sh <project_path> --cve <CVE-ID>
```
...
```bash
node scripts/dep_tree_js.js <project_path> <package_name>
```
...
```bash
bash scripts/dep_tree_go.sh <project_path> <module_path> [--target-version vX.Y.Z]
```
...
- `parent_analyses[]`: 對每個 direct parent 都下載過它 latest 版本的
  ...
  - `would_not_help_replace` — parent@latest 用 `replace` 改 target → bump_parent **無效**
    (replace directive 不繼承給 downstream consumer,見 `references/go_replace_semantics.md`)
...
`references/go_replace_semantics.md` 解釋原因,讓使用者知道這不是 last resort
而是當下唯一可行解。
...
排在最前面，即使其他 strategy 也適用。詳見 `references/go_major_version_paths.md`。
...
直接告訴使用者並建議走 `add_replace`,引用 `references/go_replace_semantics.md`。
...
**不要硬推 bump_indirect**,改走 B/Go-5。詳見 `references/go_replace_semantics.md`。
...
session 中標記 `upgrade_strategy = <chosen strategy>`，Phase 5.3 走對應命令
（見 `references/go_workflow.md` 的「升級命令 (Phase 5)」章節）。
...
```bash
node scripts/api_surface_diff_js.js <package_name> <old_version> <new_version>
```
...
```bash
bash scripts/api_surface_diff_go.sh <module_path> <old_version> <new_version>
```
...
**JavaScript path**：
```bash
bash scripts/git_diff_js.sh <git_repo_url> <current_version> <target_version>
```

**Go path**：
```bash
bash scripts/git_diff_go.sh <git_repo_url> <current_version> <target_version> [--subdir <path>]
```
...
- 詳見 `references/breaking_change_patterns_go.md`
...
如果同時滿足以下條件，**直接跳到 Phase 5**，省略 Phase 3 / 4：

- Phase 2 判定 `dependency_type == "transitive"`
- 所有 `package.json` (含 workspace) 對該 package 都是零命中
- 對所有 source 檔案 `grep -r '<pkg>'` 也是零命中（含 import / require / dynamic）

這代表：升級的是純 build-time / transitive 套件，專案完全沒有直接使用，沒有
任何 breaking change 影響面，跑 AST 掃描純粹浪費時間（IMPROVEMENTS session 實際遇到的情況）。
...
**Go path**:
```bash
go run scripts/ast_scanner_go.go <project_path> <module_path>
```
...
```bash
bash scripts/snapshot_env_js.sh <project_path> save
```

**Go path** (備份 `go.mod` / `go.sum` / `go.work*` / `vendor/modules.txt`，**不備份 vendor/ 內容**)：
```bash
bash scripts/snapshot_env_go.sh <project_path> save
```
...
詳細命令與每條 strategy 的執行步驟見 `references/go_workflow.md` 的「升級命令 (Phase 5)」。
major version path rewrite 詳見 `references/go_major_version_paths.md`。
...
```bash
bash scripts/validate_lockfile.sh <project_path>
```
...
```bash
bash scripts/validate_modfile_go.sh <project_path>
```
...
bash scripts/run_tests_js.sh <project_path> --files <source_files>

# 第二輪
bash scripts/run_tests_js.sh <project_path> --all
```

**Go path**:
```bash
# 第一輪: 只跑受影響 package（mapping 來自 ast_scanner_go 結果 → containing dir）
bash scripts/run_tests_go.sh <project_path> --files <source_files>

# 第二輪
bash scripts/run_tests_go.sh <project_path> --all

# CVE 升級建議加 -race
bash scripts/run_tests_go.sh <project_path> --all --race
```

**升完後若有 `govulncheck`**: 再跑一次 `bash scripts/govulncheck_go.sh <path> --cve <ID>
...
9. **🛡️ govulncheck 可達性分析**（如有跑）— 列出 called / imported / not_present
   findings，含 call_sites 的 file:line:function。詳見 `references/govulncheck.md`
   的「報告中如何呈現」範本。
...
# JavaScript 回退命令
bash scripts/snapshot_env_js.sh <project_path> restore
...
# Go 回退命令
bash scripts/snapshot_env_go.sh <project_path> restore
# 還原 go.mod / go.sum 後:
