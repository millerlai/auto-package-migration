...
> 🚫 **硬性規則 — 不得用既有知識代替跑 script。**
> 禁止以你對知名遷移（pydantic v1→v2、urllib3 1→2、React 17→18、Flask 1→2…）
> 的既有訓練知識，代替實際執行 Step 3.0 / 3.1 / 3.2 的 helper script。
> 「這是教科書級遷移」「我熟這個套件」**都不是**跳過任何一軌的理由。
> 你的記憶是**待驗證的假設，不是證據** —— 跑 script 取得 changelog 原文、
> git diff、API surface，才是證據。這條對應使用者的原則
> 「prefer checking over answering from memory」。
>
> 唯一合法的 Phase 3 短路是 Step 4.0 的 `zero_impact`，且必須**先跑完 AST scanner**
> 才能判定（見 Step 4.0）。除此之外，三軌（雙軌）一條都不准跳。
> 若某軌的 script 失敗或工具未裝，要在 Step 3.4 gate 如實標明 `degraded`，
> **不是**默默略過。
...
### Step 3.4: Phase 3 出場 gate（provenance 檢查）

**進入 Phase 4 之前，先在對話中貼出以下 provenance 區塊。** 這些值只有真的跑了
script 才生得出來 —— 跳過任何一軌都會在這裡留下明顯空白，而不是拖到測試階段才爆。
不要事後憑記憶補寫，要直接引用 script stdout 的 metadata header 原文。

```
## Phase 3 Provenance Gate
- Changelog (Step 3.1):   <changelog_source_url 原文> | 或 NOT_FOUND
- Git Diff  (Step 3.2):   <git_diff_compare_url> @ <old_sha[:12]>...<new_sha[:12]>
- API Surface (Step 3.0): strategy=<strategy> confidence=<confidence_score>
- 三軌狀態：changelog=[ok|degraded|missing] gitdiff=[ok|degraded] apisurface=[ok|degraded|none]
```

規則：
- 任何一軌標 `degraded` / `missing` / `none`，**必須**附一句原因（工具未裝、tag 對不上、
  repo 404…），不可空白帶過。
- 若三軌全部 `ok` 卻產出「0 個 breaking change」，要在區塊下方明寫一句
  「已實際比對三軌來源，確認無 public API 變更」——把「查證後無變更」與
  「沒查」這兩種情況分開。
- 唯一可整段省略本 gate 的情況：Step 4.0 判定 `zero_impact`（此時 Phase 3 整個跳過，
  改在報告的「## Skipped Phases」記錄原因）。

> 這裡填的來源，最後會在 Step 7.1.1 由 `verify_provenance.sh` 自動驗證（並由 Stop hook
> 再驗一次）。你在這個 gate 留的空白，到 Phase 7 會被擋下，所以不要拖到那時才補。
...
> ⚠️ **`zero_impact` 是唯一合法跳過 Phase 3 的路徑，且只在上述兩個機器可驗證的
> 條件同時成立時才適用。** 「我了解這個套件」「這是常見遷移」「應該沒影響」
> **都不構成短路條件** —— 那是憑記憶判斷，正是 Phase 3 hard rule 禁止的。
> 必須先實際跑完 AST scanner 拿到 `verdict == zero_impact`，才可短路；
> 在那之前不准以任何理由略過 Step 3.0 / 3.1 / 3.2。
...
#### Step 7.1.1: 落檔 + provenance gate（建立 PR 前必跑）

寫完報告後，**一律**把報告全文落到 canonical 路徑（這也是同一份 PR body 的來源）：

```bash
mkdir -p <project>/.package-upgrade-cache
# 把 Phase 7.1 報告全文寫到：
#   <project>/.package-upgrade-cache/migration-report.md
```

接著對它跑 provenance gate（語言無關，三語言與未來新語言共用同一支）：

```bash
bash scripts/common/verify_provenance.sh <project>/.package-upgrade-cache/migration-report.md
```

- `exit 0` → provenance 完整（或 `zero_impact` 合法跳過），進 Step 7.2。
- `exit 1` → stdout JSON 的 `missing[]` 列出缺哪幾軌的來源。**不要硬著頭皮建 PR**，
  回去把缺的軌（Step 3.1 / 3.2 / 3.0）實際跑完、把來源補進報告，重跑直到通過。
  這正是擋下「憑記憶跳過 Phase 3」的最後一道 deterministic 關卡 —— 同樣的檢查
  也由 Stop hook（`provenance_stop_hook.py`，install 時寫入 settings.json）在你結束
  對話時自動再跑一次。
