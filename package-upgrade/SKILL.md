...
- `is_workspace`: 是否為 monorepo
- `has_typescript`: 是否有 TS
- `tsconfig_path`: tsconfig.json 路徑
- `types_entry`: package.json 中宣告的 types entry
...
- `yarn` / `pnpm` / `bun` → MVP 階段尚未支援，向使用者說明並詢問是否 fallback
- 並讀 `references/js_ast_strategy.md` 與 `references/breaking_change_patterns_js.md`

**MVP 限制**（遇到時暫停告知使用者）：
- `is_workspace: true` → 暫不處理 monorepo workspace 升級傳播，請使用者指定要升的 workspace 範圍
- `pkg_manager` 非 `npm` → MVP 只實作 npm，其他工具尚未支援
