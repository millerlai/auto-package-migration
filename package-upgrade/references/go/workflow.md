...
> `major_version_paths.md` + `breaking_change_patterns.md` + `govulncheck.md`（同資料夾）。
...
|------------|-----------|
| 基本流程 | 本檔 `workflow.md` |
| Major version (`v2+`) 升級 | `major_version_paths.md` |
| CVE / 漏洞分析 | `govulncheck.md` |
| Breaking change pattern | `breaking_change_patterns.md` |
| Auth (`.netrc`, GOPROXY) | `../common/auth_tokens.md` |
| BDSA / GHSA → CVE | `../common/bdsa_mapping.md` |
| Jira / commit / PR | `../common/jira_workflow.md` |
...
**這是 Go 升級最特殊的場景**。詳見 `major_version_paths.md`。摘要：
...
# 2. 用 scripts/go/ast_scanner.go 找出所有 `"example.com/foo"` import,
...
## Phase 2 升級策略（scripts/go/dep_tree.sh 輸出）
...
```bash
bash scripts/go/govulncheck.sh <project_path> [--cve CVE-XXXX-XXXXX]
```
