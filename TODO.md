...
- `detect_env_js.sh` 的 missing-`package.json` 錯誤訊息把 `$PROJECT_PATH` 直接 inline 進 JSON，Windows backslash 路徑（如 `C:\Users\...`）會產生無效的 JSON escape（`\U` / `\T` 等）。POSIX 不受影響。修法：改用 `jq -Rs` 編碼路徑。
