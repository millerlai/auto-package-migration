...
if [ ! -f "$GRANT_SCRIPT" ]; then
    ...
    echo ""
    echo "另會寫入一個 Stop hook (provenance_stop_hook.py) 到 settings.json:"
    echo "  - 結束對話時自動驗證遷移報告是否含 Phase 3 三軌來源,缺漏時擋下並提示補跑"
    echo "  - 只在偵測到 .package-upgrade-cache/migration-report.md 時作用,不影響其他工作"
    echo "  - 若不想要,稍後可加 --no-hooks 重跑 grant_permissions.py"
    echo ""
